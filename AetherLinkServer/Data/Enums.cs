...
namespace AetherLinkServer.Data
{
    ...
    public static class JobWithRoleHelper
    {
        ...
        public static Dictionary<Enums.JobWithRole, IEnumerable<Enums.JobWithRole>> categories = enumVals.Select(jwr => (jwr, enumVals.Where(jwrr => jwr != jwrr && jwr.HasFlag(jwrr)))).Where(j => j.Item2.Any())
                                                                                             .ToDictionary(j => j.jwr, j => j.Item2);

        public static Dictionary<Enums.JobWithRole, IEnumerable<Enums.JobWithRole>> values = enumVals.Select(jwr => (jwr, enumVals.Where(jwrr => jwr != jwrr && jwrr.HasFlag(jwr)))).Where(j => j.Item2.Any())
                                                                                         .ToDictionary(j => j.jwr, j => j.Item2);
        ...
        public static void DrawCategory(Enums.JobWithRole category, ref Enums.JobWithRole config, bool allowRemoval = true)
        {
            ...
            {
                ImGui.Indent();
                foreach (Enums.JobWithRole jobW in categories[category])
